...
void _TextList::Init(void)
{
  ...
  HostLanguagePushPopCnt = 0;
  HostLanguageNo     = 0xFFFFFFFF;  
...
_BOOL _TextList::HostLanguageSet(_UDWORD language)
{
  ...
  if(language == HostLanguageNo)
  {
    return _TRUE; // hostlanguage ist bereits gesetzt
  }
  else
  {
    HostLanguageNo = 0xFFFFFFFF;
    if(language < anz)
    {
      if(SwapLanguage(_TRUE, language) == _TRUE)
      {
        HostLanguageNo = language;
        SwapLanguage(_FALSE, 0);
      }
    }
  }
...
_BOOL _TextList::HostLanguagePush(void)
{
  if(HostLanguageNo < Project.GetLanguageNo())
  {
    if(SwapLanguage(_TRUE, HostLanguageNo) == _TRUE)
    {
      HostLanguagePushPopCnt++;
      return _TRUE;
    }
  }
  return _FALSE;
...
_BOOL _TextList::HostLanguagePop(void)
{
  if(HostLanguagePushPopCnt > 0)
  {
    HostLanguagePushPopCnt--;
    if(HostLanguagePushPopCnt == 0)
    {
      return SwapLanguage(_FALSE, 0);
    }
  }
  return _FALSE;
}
...
{
  return TextList.HostLanguagePush();
}

extern "C" _BOOL Prj_HostLanguagePop(void)
{
  return TextList.HostLanguagePop();
}

extern "C" _BOOL Prj_HostLanguageSet(_UDWORD language)
{
  return TextList.HostLanguageSet(language);
}
